...
class DNet(nn.Module):
    ...
    def __init__(self, num_class=NUM_CLASS):
        ...
        self.logit = nn.ModuleList(
            #[ nn.Linear(1024,c) for c in num_class ]
...
class Net(nn.Module):
    ...
    def __init__(self, num_class=NUM_CLASS):
        super(Net, self).__init__()
        self.e = EfficientNet.from_pretrained('efficientnet-b3')

        self.logit = nn.ModuleList(
            [ nn.Linear(1000,c) for c in num_class ]
        )
...
def cross_entropy_ohem_loss(logit, onehot):
    rate = 0.75
    ...
    loss = (log_probability*onehot)
    loss = loss.sum(1)
    keep_num = min(loss.size()[0], int(batch_size*rate))
    sorted_ohem_loss, idx = torch.sort(loss, descending=True)
    if keep_num < loss.size()[0]:
...
def criterion(logit, truth):
    ...
    for i in range(3):
        ...
        #e = cross_entropy_ohem_loss(logit[i], truth[i])
