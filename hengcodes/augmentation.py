...
def do_random_pad_crop(image, border):
    height,width = image.shape[:2]
    image = cv2.copyMakeBorder(image, border, border, border, border, borderType=cv2.BORDER_CONSTANT, value=(0,0,0))
    x = np.random.choice(2*border)
    y = np.random.choice(2*border)
...
def do_random_batch_cutmix(input, onehot):
    ...
    inv_mask = np.zeros((batch_size,C,H,W), np.float32)
    ...
    for b in range(batch_size):
        ...
        inv_mask[b, :, y0: y1, x0: x1] = 1

    mask  = torch.from_numpy(mask).to(input.device)
    inv_mask  = torch.from_numpy(inv_mask).to(input.device)
    ...
    mix_input  = mask*input + inv_mask*perm_input
